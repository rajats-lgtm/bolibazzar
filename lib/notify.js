...
/** Deliver a login code over whichever channel we have. */
export async function sendOtp(db, { destination, channel, code }) {
  ...
  if (channel === 'phone') {
    try {
      ...
      return { channel: 'whatsapp', error: error.message };
    }
  }
  // Email transport is not wired up yet; log it so local dev still works.
  console.log('[otp:email] %s -> %s', destination, body);
