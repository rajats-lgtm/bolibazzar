...
/**
 * Secure cookies are only sent back over HTTPS, so setting the flag on a
 * plain-HTTP deployment silently breaks every login. Derive it from the scheme
 * we actually serve on rather than from how the build was compiled.
 */
...
/**
 * Which deployment this is, as opposed to how the code was compiled.
 *
 * A UAT runs a production *build* (NODE_ENV=production) but is not a
 * production *environment*: testers still need to sign in without an SMS
 * gateway. APP_ENV states the intent explicitly, and when it is unset we fall
 * back to NODE_ENV so the safe answer is the default.
 */
...
/** True when OTPs may be returned in the API response. Never in production. */
