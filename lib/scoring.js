...
/**
 * Shipped tier table. Thresholds and cashback are settings an admin can change,
 * so this is the fallback used when the caller has no resolved settings to hand
 * — see `tierTable()` in lib/settings.js for the live values.
 */
export const TIERS = [
...
/**
 * @param totalSpent lifetime spend in rupees
 * @param tiers      the live tier table; defaults to the shipped one
 */
export function computeTier(totalSpent, tiers = TIERS) {
  const spent = Number(totalSpent) || 0;
  const table = Array.isArray(tiers) && tiers.length ? tiers : TIERS;
  const index = table.findIndex((t) => spent >= t.min);
  const current = table[index];
  const next = index > 0 ? table[index - 1] : null;
  return {
    tier: current.tier,
    label: current.label,
