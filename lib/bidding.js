 ...
 *                       work with no live humans. Switched off in the admin
 *                       console, or by DEMO_BIDDERS=false.
 *
 ...
 *
 * The window length and whether demo bidders run are settings, so an admin can
 * change them on a running system. They are read per auction rather than held
 * in a module constant, which is why the helpers below are async.
 */

/** Fallback used only when settings cannot be read at all. */
export const DEFAULT_AUCTION_WINDOW_MS = 120 * 1000;

export async function auctionWindowMs() {
  const { auction_window_seconds: seconds } = await getSettings();
  return Number(seconds || 120) * 1000;
}

export async function demoBiddersEnabled() {
  const { demo_bidders_enabled: enabled } = await getSettings();
 ...
 * the auction start.
 */
export function planDemoBids(request, suppliers, windowMs = DEFAULT_AUCTION_WINDOW_MS) {
  ...
  return chosen.map((supplier, index) => {
    ...
    for (let d = 0; d < dropCount; d++) {
      const at = arriveAt + (0.35 + d * 0.25 + rng() * 0.2) * windowMs;
...
export async function advanceAuction(db, request) {
  // Resolved once per call: a window that changed mid-auction must not make
  // "live" and "ends_at" disagree within the same response.
  const settings = await getSettings();
  const windowMs = Number(settings.auction_window_seconds || 120) * 1000;
  const demoEnabled = settings.demo_bidders_enabled !== false;
  ...
  if (demoEnabled && request.status === 'open') {
    ...
    if (demoSuppliers.length) {
      const plans = planDemoBids(request, demoSuppliers, windowMs);
...
export async function runAutoBidMatching(db, request) {
  const settings = await getSettings();
  if (settings.auto_bid_enabled === false) return [];
