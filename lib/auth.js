...
/** Read the session subject (email) for a role from the request cookies. */
export function sessionSubject(req, role) {
  const raw = req.cookies?.get?.(cookieName(role))?.value || '';
  const payload = verifySessionToken(role, raw);
